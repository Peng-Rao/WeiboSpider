...
def reverse_cut_to_length(content, code_func, cut_num=4, fill_num=7):
    ...
    content = str(content)
    cut_list = [
        content[i - cut_num if i >= cut_num else 0 : i]
        for i in range(len(content), 0, (-1 * cut_num))
    ]
    cut_list.reverse()
    result = []
...
def parse_tweet_info(data):
    ...
    tweet = {
        ...
        "pic_urls": [
            "https://wx1.sinaimg.cn/orj960/" + pic_id
    ...
    if "page_info" in data and data["page_info"].get("object_type", "") == "video":
        ...
        if "media_info" in data["page_info"]:
            ...
            "cards" in data["page_info"]
            and "media_info" in data["page_info"]["cards"][0]
        ):
