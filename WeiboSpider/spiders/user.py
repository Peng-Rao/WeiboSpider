...
class UserSpider(Spider):
    ...
    def start_requests(self):
        ...
        user_ids = ["1749127163"]
        urls = [
            f"https://weibo.com/ajax/profile/info?uid={user_id}" for user_id in user_ids
        ]
        for url in urls:
    ...
    def parse_detail(response):
        ...
        item["label_desc"] = str(
            [label["name"] for label in data.get("label_desc", [])]
        )
        if "company" in data:
            item["company"] = data["company"]
        if "education" in data:
            item["education"] = data["education"].get("school", "")
        yield item
